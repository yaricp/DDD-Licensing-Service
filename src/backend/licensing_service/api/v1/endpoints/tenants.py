...
from ..services.tenant import (
    get_tenant, create_tenant, get_all_tenants,
...
@router.post(
    "/", name="Create a new Tenant", response_model=Tenant
)
...
@router.get(
    "/", name="Get All Tenants", response_model=List[Tenant]
)
...
@router.get(
    "/{id}", name="Get Tenant", response_model=Tenant
)
...
@router.put(
    "/{id}", name="Update Tenant", response_model=Tenant
)
async def update_tenant_route(
    id: UUID, item_in: TenantUpdate,
    messagebus_handler=Depends(get_messagebus_handler)
):
...
@router.delete(
    "/{id}", name="Delete Tenant", response_model=Tenant
)
async def delete_tenant_route(
    id: UUID,
    messagebus_handler=Depends(get_messagebus_handler)
):
    tenant: Tenant = await delete_tenant(
