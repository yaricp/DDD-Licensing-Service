...
class TenantService:
    ...
    def __init__(
        ...
        if db_session_factory:
            self._uow: TenantUnitOfWork = UOW(session_factory=db_session_factory)
        else:
    ...
    async def create_tenant(
        ...
        async with self._uow as uow:
            tenant = Tenant.make(
                ...
                address=create_command.address,
                phone=create_command.phone,
            )
            created_tenant = await uow.tenants.save(tenant=tenant)
            await uow.commit()
            tenant = await self.get_tenant_by_id(id=created_tenant.id)
    ...
    async def update_tenant(self, update_command: UpdateTenantCommand) -> Tenant:
        async with self._uow as uow:
            ...
            tenant.update(
                update_command.name,
                update_command.address,
                update_command.email,
                update_command.phone,
            )
            tenant: Tenant = await uow.tenants.update(id=tenant.id, model=tenant)
            await uow.commit()
