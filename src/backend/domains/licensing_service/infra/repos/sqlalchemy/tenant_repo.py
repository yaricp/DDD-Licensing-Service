...
class SQLAlchemyTenantRepository(SQLAlchemyAbstractRepository, TenantRepository):

    async def get(self, id: UUID) -> Optional[Tenant]:
        result: Result = await self._session.execute(
            select(Tenant)
            .options(
                selectinload(Tenant.subdivisions).selectinload(Subdivision.licenses),
                selectinload(Tenant.subdivisions).selectinload(Subdivision.statistics),
        ...
        tenant = Tenant.make_from_persistence(
            ...
            phone=tenant.phone,
            users=tenant.users,
            subdivisions=tenant.subdivisions,
            id=tenant.id,
        )
    ...
    async def add(self, model: AbstractEntity) -> Tenant:
        ...
        result: Result = await self._session.execute(
            ...
            .values(
                **await model.to_dict(
                    exclude={"id", "users", "_domain_events", "subdivisions"}
                )
            )
            .returning(Tenant)
        )
    ...
    async def update(self, id: UUID, model: AbstractEntity) -> Tenant:
        ...
        result: Result = await self._session.execute(
            update(Tenant).filter_by(id=id).values(**to_save).returning(Tenant)
        )
    ...
    async def list(self) -> List[Tenant]:
        ...
        for db_tenant in db_tenants:
            ...
            tenant = Tenant.make_from_persistence(
                ...
                users=db_tenant.users,
                subdivisions=db_tenant.subdivisions,
            )
