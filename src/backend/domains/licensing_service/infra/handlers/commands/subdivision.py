...
from ....domain.services.handlers.subdivision_handlers import SubdivisionCommandHandler


class ActivateSubdivisionLicenseCommandHandler(SubdivisionCommandHandler):

    async def __call__(self, command: ActivateSubdivisionLicenseCommand) -> Subdivision:
        """
...
class AddStaticticRowSubdivisionCommandHandler(SubdivisionCommandHandler):
    async def __call__(self, command: AddStatisticRowCommand) -> Subdivision:
        """
...
class CreateSubdivisionCommandHandler(SubdivisionCommandHandler):
    ...
    async def __call__(self, command: CreateSubdivisionCommand) -> Subdivision:
        """
...
class UpdateSubdivisionCommandHandler(SubdivisionCommandHandler):
    ...
    async def __call__(self, command: UpdateSubdivisionCommand) -> Subdivision:
        """
        Registers a new user, if user with provided credentials doesn't exist,
        and creates event signaling that
        operation was successfully executed.
...
class DeleteSubdivisionCommandHandler(SubdivisionCommandHandler):
    ...
    async def __call__(self, command: DeleteSubdivisionCommand) -> Subdivision:
        """
        ...
        subdivision = await subdivision_service.delete_subdivision(id=command.id)
        return subdivision
