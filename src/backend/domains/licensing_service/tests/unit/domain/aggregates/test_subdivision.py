...
from backend.core.infra.eventbus import AbstractEvent, AbstractEventBus
...
async def test_make_subdivision():
    ...
    """
    subdivision = Subdivision.make(name="Test", location="Somewhere", tenant_id=uuid4())
...
async def test_activate_license_subdivision():
    ...
    with pytest.raises(LicenseNotFoundError):
        await subdivision.activate_license(license_id=100000, eventbus=domain_event_bus)
    await subdivision.activate_license(license_id=license.id, eventbus=domain_event_bus)
...
async def test_save_day_statistic_subdivision():
    ...
    with pytest.raises(SubdivisionInactiveError):
        await subdivision.save_day_statistic(
            StatisticRow.make(count_requests=15, subdivision_id=subdivision.id),
    ...
    with pytest.raises(SubdivisionStatisticAlreadyExistsError):
        await subdivision.save_day_statistic(statistic_row, eventbus=domain_event_bus)
