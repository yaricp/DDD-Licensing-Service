...
from backend.domains.licensing_service.domain.services.commands.subdivision_commands import (
    CreateSubdivisionCommand, UpdateSubdivisionCommand, AddStatisticRowCommand
)
...
from backend.domains.licensing_service.domain.services.events.subdivision_events import (
    SubdivisionCreatedEvent, SubdivisionUpdatedEvent, SubdivisionDeletedEvent,
...
from backend.domains.licensing_service.app.services.tenant_services import (
    TenantService
)
...
from backend.domains.licensing_service.domain.exceptions.subdivision import (
    SubdivisionNotFoundError, SubdivisionAlreadyExistsError,
...
async def test_add_license_to_subdivision(db_session):
    ...
    subdivision_cmd = CreateSubdivisionCommand(
        name="License Test Subdivision",
        location="License City",
        tenant_id=tenant.id
    )
...
async def test_update_license_in_subdivision(db_session):
    ...
    subdivision_cmd = CreateSubdivisionCommand(
        name="Update License Subdivision",
...
async def test_delete_license_from_subdivision(db_session):
    ...
    subdivision_cmd = CreateSubdivisionCommand(
        name="Delete License Subdivision",
...
@pytest.mark.asyncio
async def test_subdivision_operations_with_nonexistent_subdivision_id(
    db_session
):
    """
...
async def test_update_subdivision_with_all_fields(db_session):
    ...
    updated_subdivision = await subdivision_service.update_subdivision(
        update_cmd
    )
    ...
    assert updated_subdivision.link_to_subdivision_processing_domain == "https://new-link.example.com"

    # Verify event was published
    update_events = [
        e for e in infra_event_bus.events if isinstance(
            e, SubdivisionUpdatedEvent
        )
    ]
...
async def test_add_statistic_row_to_subdivision_with_license(db_session):
    ...
    stat_added_subdivision = await subdivision_service.add_subdivision_statistic_row(stats_cmd)
...
async def test_add_statistic_row_deactivate_subdivision(db_session):
    ...
    assert isinstance(infra_event_bus.events[-2], StatisticRowAddedEvent)
    assert isinstance(
        infra_event_bus.events[-1], SubdivisionLicenseExpiredEvent
    )
